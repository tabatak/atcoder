...
func main() {
	...
	ans := 1
	index := 0
	var isBeforeVertical bool
	if ss[0][0] == ss[1][0] {
		// 先頭が縦
		ans *= 3
		index = 1
		isBeforeVertical = true
	} else {
		// 先頭が横
		ans *= 6
		index = 2
		isBeforeVertical = false
	}
	...
	for index < n {
		...
		if !isBeforeVertical && isVertical {
			ans *= 1
		} else if !isBeforeVertical && !isVertical {
			ans *= 3
			ans %= mod
		} else {
			ans *= 2
