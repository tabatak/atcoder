...
func main() {
	...
	for i := 0; i < Q; i++ {
		...
		// xに最も近い神社2つ
		nsIndex := sort.SearchInts(ss, x)
		nss := []int{ss[nsIndex], ss[nsIndex-1]}
		// xに最も近い寺2つ
		ntIndex := sort.SearchInts(ts, x)
		nts := []int{ts[ntIndex], ts[ntIndex-1]}

		ans := math.MaxInt64
		for _, s := range nss {
			for _, t := range nts {
				d := min(abs(x-s)+abs(s-t), abs(x-t)+abs(t-s))
				ans = min(ans, d)
			}
		}
