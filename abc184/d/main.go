...
func main() {
	...
	dp := make([][][]float64, 101)
	for i := 0; i < 101; i++ {
		tmp := make([][]float64, 101)
		for j := 0; j < 101; j++ {
			tmp[j] = make([]float64, 101)
		}
		dp[i] = tmp
	}
	dp[a][b][c] = float64(1)
		...
		for j := b; j < 101; j++ {
			for k := c; k < 101; k++ {
				if i == 100 || j == 100 || k == 100 {
					trialCount := (i + j + k) - (a + b + c)
					ans += float64(trialCount) * dp[i][j][k]
					continue
				}
				...
				jf64 := float64(j)
				kf64 := float64(k)
				dp[i+1][j][k] += dp[i][j][k] * if64 / (if64 + jf64 + kf64)
				dp[i][j+1][k] += dp[i][j][k] * jf64 / (if64 + jf64 + kf64)
				dp[i][j][k+1] += dp[i][j][k] * kf64 / (if64 + jf64 + kf64)
			}
		}
	}
