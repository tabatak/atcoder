...
func main() {
	...
	maxNum := 100000
	notPrimes := make([]bool, maxNum+1)
	notPrimes[0] = true
	notPrimes[1] = true
	for num := 2; num*num <= maxNum; num++ {
		if !notPrimes[num] {
			for m := 2; num*m <= maxNum; m++ {
				notPrimes[num*m] = true
			}
		}
	}
	...
	for i := 3; i <= maxNum; i += 2 {
		if !notPrimes[i] && !notPrimes[(i+1)/2] {
			c[i]++
		}
	}
	for i := 3; i < maxNum; i++ {
