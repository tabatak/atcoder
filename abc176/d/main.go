...
func main() {
	...
	for ph.Len() > 0 {
		...
		for i := range dx {
			...
			ny := p.y + dy[i]
			// 探索対象ならキューに追加
			if nx < 0 || nx >= W || ny < 0 || ny >= H || matrix[ny][nx] == '#' {
				continue
			}
			if d[ny][nx] <= p.c {
				continue
...
func (h pheap) Less(i, j int) bool { return h[i].c < h[j].c }
