...
func main() {
	...
	holidays := make(map[holiday]bool)
	var line string
	for i := 0; i < n; i++ {
		fmt.Fscan(r, &line)
		md := strings.Split(line, "/")
		m, _ := strconv.Atoi(md[0])
		d, _ := strconv.Atoi(md[1])
		holidays[holiday{m, d}] = true
	}

	month := 1
	day := 1
	dayOfWeek := 0
	isWeekend := []bool{true, false, false, false, false, false, true}
	ans := 0
	tmp := 0
	hurikae := 0
	for i := 1; i <= 366; i++ {
		// 週末 or 祝日 or 振替
		if isWeekend[dayOfWeek] || holidays[holiday{month, day}] {
			tmp++
		} else if hurikae > 0 {
			tmp++
			hurikae--
...
func nextDay(month, day int) (int, int) {
	if month == 2 {
		if day == 29 {
			return month + 1, 1
		}
