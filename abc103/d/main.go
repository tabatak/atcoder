...
func main() {
	...
	for i := 0; i < m; i++ {
		fmt.Fscan(r, &a)
		fmt.Fscan(r, &b)
		heap.Push(rh, relation{a, b})
	}

	ans := 0
	disconnected := 0
	for i := 0; i < m; i++ {
		rel := heap.Pop(rh).(relation)

		// fmt.Printf("%d - %d\n", rel.a, rel.b)
		if rel.a > disconnected {
			disconnected = rel.b - 1
			ans++
		}
	}
...
func (h *relHeap) Push(x interface{}) {
	*h = append(*h, x.(relation))
}
